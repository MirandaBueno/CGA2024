...
GLuint textureEsponjaID, textureWaterID;
...
glm::mat4 modelMatrixArcade = glm::mat4(1.0f);
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	/*TAREA 3*/
	boxEsponja.init();
	boxEsponja.setShader(&shaderMulLightingMT);
	...
	modelDartLegoHead.loadModel("../models/buzz/buzzlightyHead.obj");
	...
	/*MODELOS PROPPIOS*/
	modelPelota.loadModel("../models/pelota/pelota.obj");
	modelPelota.setShader(&shaderMulLighting);
	modelArcade.loadModel("../models/arcade/arcade.obj");
	modelArcade.setShader(&shaderMulLighting);
	...
	if (textureEsponja.getData()) {
		...
		// Formato interno de la libreria de la imagen, el tipo de dato y al apuntador
		// a los datos
		std::cout << "Numero de canales :=> " << textureEsponja.getChannels() << std::endl;
...
void applicationLoop() {
	...
	/*MODELOS PROPIOS*/
	modelMatrixPelota = glm::translate(modelMatrixPelota, glm::vec3(-10.0, 0.0, 2.0));
	modelMatrixArcade = glm::translate(modelMatrixArcade, glm::vec3(-15.0, 0.0, 2.0));
	...
	while (psi) {
		...
		shaderMulLighting.setMatrix4("view", 1, false,
			glm::value_ptr(view));
		// Settea las matriz de MVP al shader con varias texturas
		...
		*********************************************/
		...
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, textureWaterID); // Cambiar a water
		shaderMulLightingMT.setInt("texture2", 0);
		glActiveTexture(GL_TEXTURE3);
		glBindTexture(GL_TEXTURE_2D, textureEsponjaID); // Cambiar a sponge
		shaderMulLightingMT.setInt("texture1", 3);
		boxEsponja.setScale(glm::vec3(5.0, 5.0, 0.05));
		boxEsponja.setPosition(glm::vec3(0.0, 2.5, -20.0));
		boxEsponja.setOrientation(glm::vec3(0.0, 0.0, 0.0));
		boxEsponja.render();

		boxEsponja.setScale(glm::vec3(5.0, 5.0, 0.05));
		boxEsponja.setPosition(glm::vec3(2.5, 2.5, -17.5));
		boxEsponja.setOrientation(glm::vec3(0.0, 90.0, 0.0));
		boxEsponja.render();

		boxEsponja.setScale(glm::vec3(5.0, 5.0, 0.05));
		boxEsponja.setPosition(glm::vec3(0.0, 2.5, -15.0));
		boxEsponja.setOrientation(glm::vec3(0.0, 0.0, 0.0));
		boxEsponja.render();
		...
		boxEsponja.setScale(glm::vec3(5.0, 5.0, 0.05));
		boxEsponja.setPosition(glm::vec3(0.0, 5, -17.5));
		boxEsponja.setOrientation(glm::vec3(90.0, 0.0, 0.0));
		boxEsponja.render();
		...
		*******************************************/
		...
		/*MODELOS PROPIOS*/
		glm::mat4 modelMatrixPelota2 = glm::mat4(modelMatrixPelota);
		modelMatrixPelota2 = glm::scale(modelMatrixPelota, glm::vec3(0.03, 0.03, 0.03));
		modelPelota.render(modelMatrixPelota2);

		glm::mat4 modelMatrixArcade2 = glm::mat4(modelMatrixArcade);
		modelMatrixArcade2 = glm::scale(modelMatrixArcade, glm::vec3(2.5, 2.5, 2.5));
		modelArcade.render(modelMatrixArcade2);
		...
		modelMatrixDartHead = glm::scale(modelMatrixDartHead, glm::vec3(10.0, 10.0, 10.0));
